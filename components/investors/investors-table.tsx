  ...
  if (investors.length === 0) {
    return (
      <div className="rounded-md border border-border bg-card p-8 text-center">
        <h2 className="text-lg font-semibold text-foreground">Nenhum cliente ainda.</h2>
        <p className="mb-6 mt-2 text-sm text-muted-foreground">
          Crie um perfil de cliente/investidor ou adicione demos para testar os workspaces.
        </p>
        <InvestorFormModal />
      </div>
    )
      ...
      <div className="divide-y divide-border/70">
        {investors.map((investor) => (
          ...
          >
            ...
            <Badge variant="outline">{STRATEGY_LABELS[investor.strategy] ?? investor.strategy}</Badge>
            <Badge variant="outline">{RISK_LABELS[investor.risk_level] ?? investor.risk_level}</Badge>
